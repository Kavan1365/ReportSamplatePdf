...
namespace KaUI.Utilities
{
    public static class ModelBuilderExtensions
    {
        ...
        //{
        //    modelBuilder.AddDefaultValueSqlConvention("Guid", typeof(Guid), "NEWSEQUENTIALID()");
        //}
        ...
        /// <param name="propertyType">Type of property wants to set DefaultValueSql for </param>
        /// <param name="defaultValueSql">DefaultValueSql like "NEWSEQUENTIALID()"</param>
        public static void AddDefaultHasIndexSqlConvention(this ModelBuilder modelBuilder)
        {
            foreach (IMutableEntityType entityType in modelBuilder.Model.GetEntityTypes())
            {
                entityType.AddAnnotation("HasIndex", "Guid");
            ...
            foreach (Type type in types)
            {
                foreach (Type iface in type.GetInterfaces())
                {
                    if (iface.IsConstructedGenericType && iface.GetGenericTypeDefinition() == typeof(IEntityTypeConfiguration<>))
                    {
                        ...
                        var sds = Activator.CreateInstance(type);
        ...
        public static void RegisterAllEntities<BaseType>(this ModelBuilder modelBuilder, params Assembly[] assemblies)
        {
            ...
            foreach (Type type in types.Where(x => x.Name != "BaseEntity"))
